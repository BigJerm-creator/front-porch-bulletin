...
import { Save, Building2, Eye, EyeOff, Clock, AlertTriangle } from "lucide-react";
...
export default function AdminBusinessSpotlight() {
  ...
  const publishSpotlight = usePublishBusinessSpotlight();
  ...
  const handleToggle = async () => {
    try {
      ...
      if (isDisabled) {
        toast({ title: "Spotlight enabled", description: "The business spotlight is now live." });
      } else if (isPendingDisable) {
        toast({ title: "Stage cancelled", description: "The business spotlight will remain on the current edition." });
      } else {
  ...
  const handlePublish = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getGetBusinessSpotlightQueryKey() });
      toast({ title: "Spotlight hidden", description: "The business spotlight has been removed from the public edition." });
    } catch {
