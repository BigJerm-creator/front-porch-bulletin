...
export default function AdminRoute() {
  const { data: roleData, isLoading, isError } = useGetMyRole({
    query: { queryKey: getGetMyRoleQueryKey(), retry: false },
  });
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 font-serif text-foreground">
        <div className="max-w-lg text-center border-4 border-foreground p-12 bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          ...
          <h2 className="text-4xl font-headline font-bold uppercase tracking-widest mb-6 text-foreground">
            Pending Approval
          </h2>
          ...
          <p className="text-xl italic leading-relaxed text-muted-foreground mt-6 mb-8">
            Your credentials have been received by the editorial desk, but you must be cleared
            by the Editor-in-Chief before accessing the staff room.
          </p>
