...
import { useGetSpotlight, useUpdateSpotlight, useToggleSpotlight, usePublishSpotlight, getGetSpotlightQueryKey } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Skeleton } from "@/components/ui/skeleton";
import { useToast } from "@/hooks/use-toast";
import { Save, User, Eye, EyeOff, Clock, AlertTriangle } from "lucide-react";
...
export default function AdminSpotlight() {
  ...
  const handleToggle = async () => {
    try {
      ...
      if (isDisabled) {
        toast({ title: "Spotlight enabled", description: "The student spotlight is now live." });
      } else if (isPendingDisable) {
        toast({ title: "Stage cancelled", description: "The student spotlight will remain on the current edition." });
      } else {
  ...
  const handlePublish = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getGetSpotlightQueryKey() });
      toast({ title: "Spotlight hidden", description: "The student spotlight has been removed from the public edition." });
    } catch {
  ...
  const handleSave = async () => {
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Failed to save spotlight." });
    }
  };
  ...
  return (
    <div className="space-y-8">
      <header className="border-b-4 border-foreground pb-6 mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          <h1 className="text-4xl sm:text-5xl font-headline font-bold uppercase tracking-widest mb-2">Student Spotlight</h1>
          <p className="text-xl text-muted-foreground italic font-serif">Feature a student on the front page.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-6 border-4 border-foreground bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6">
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest mb-1">Student Name</label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-1">School</label>
                <Input
                  className="rounded-none border-2 border-foreground font-serif text-base"
                  value={form.school}
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-1">Grade</label>
                <Input
                  className="rounded-none border-2 border-foreground font-serif text-base"
                  value={form.grade}
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-foreground p-4 bg-[#f5f0e8]">
              <p className="text-xs font-mono uppercase tracking-widest font-bold border-b border-foreground pb-1 mb-3">Student Spotlight</p>
              <div
              ...
              >
                ...
                <div className="w-full bg-foreground/80 text-background text-center text-xs font-mono uppercase tracking-wider py-1">Photo</div>
              </div>
              <p className="text-xs font-mono uppercase tracking-widest text-foreground/60 mb-1 text-center">Student of the Week</p>
