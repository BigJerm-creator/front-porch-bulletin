...
async function isEmpty(table: Parameters<typeof db.select>[0] extends undefined ? any : any, tableName: string): Promise<boolean> {
  const result = await db.execute(sql.raw(`SELECT COUNT(*)::int AS n FROM ${tableName}`));
  const n = (result.rows[0] as any)?.n ?? 1;
  return Number(n) === 0;
}
...
export async function seedIfEmpty(): Promise<void> {
  ...
  // ── Articles — only seed on a fresh (empty) table ────────────────────────────
  if (await isEmpty(articlesTable, "articles")) {
...
This is a letter from the editor to express our gratitude to all the patrons who have helped to build this into what it is. ',
  ...
  // ── Churches — only seed on a fresh (empty) table ────────────────────────────
  if (await isEmpty(churchesTable, "churches")) {
    logger.info("Seeding initial churches...");
    await db.execute(sql`
  ...
  // ── Student Spotlight — only seed on a fresh (empty) table ───────────────────
  if (await isEmpty(spotlightTable, "student_spotlight")) {
    logger.info("Seeding initial student spotlight...");
    await db.execute(sql`
...
Natalee Deckard, Amy Ortega, Cheyanna Morgan, Hayden Wentworth, Josie Enkey',
  ...
  // ── Business Spotlight — only seed on a fresh (empty) table ─────────────────
  if (await isEmpty(businessSpotlightTable, "business_spotlight")) {
    logger.info("Seeding initial business spotlight...");
    await db.execute(sql`
  ...
  // ── Group Spotlight — only seed on a fresh (empty) table ────────────────────
  if (await isEmpty(groupSpotlightTable, "group_spotlight")) {
    logger.info("Seeding initial group spotlight...");
    await db.execute(sql`
  ...
  // ── Calendar Events — only seed on a fresh (empty) table ────────────────────
  if (await isEmpty(calendarEventsTable, "calendar_events")) {
    logger.info("Seeding initial calendar events...");
    await db.execute(sql`
  ...
  await db.execute(sql`
    SELECT setval('articles_id_seq',          GREATEST((SELECT MAX(id) FROM articles), 1));
    SELECT setval('categories_id_seq',        GREATEST((SELECT MAX(id) FROM categories), 1));
    SELECT setval('churches_id_seq',          GREATEST((SELECT MAX(id) FROM churches), 1));
    SELECT setval('calendar_events_id_seq',   GREATEST((SELECT MAX(id) FROM calendar_events), 1));
    SELECT setval('student_spotlight_id_seq', GREATEST((SELECT MAX(id) FROM student_spotlight), 1));
    SELECT setval('business_spotlight_id_seq',GREATEST((SELECT MAX(id) FROM business_spotlight), 1));
    SELECT setval('group_spotlight_id_seq',   GREATEST((SELECT MAX(id) FROM group_spotlight), 1));
    SELECT setval('user_roles_id_seq',        GREATEST((SELECT MAX(id) FROM user_roles), 1));
  `);
